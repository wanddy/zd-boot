package webapi.quartz.service.impl;
...
import webapi.quartz.mapper.QuartzJobMapper;
...
public class QuartzJobServiceImpl extends ServiceImpl<QuartzJobMapper, QuartzJob> implements IQuartzJobService {
	...
	private void schedulerAdd(String jobClassName, String cronExpression, String parameter) {
		try {
			...
			throw new ZdException("创建定时任务失败", e);
		} catch (RuntimeException e) {
			throw new ZdException(e.getMessage(), e);
		}catch (Exception e) {
			throw new ZdException("后台找不到该类名：" + jobClassName, e);
		}
	}
	...
	private void schedulerDelete(String jobClassName) {
		try {
			...
			throw new ZdException("删除定时任务失败");
		}
	}
