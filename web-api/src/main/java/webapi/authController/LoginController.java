...
import auth.discard.model.SysLoginModel;
import auth.discard.util.RandImageUtil;
...
import commons.util.*;
import commons.util.encryption.EncryptedString;
...
public class LoginController {
    ...
    @Autowired
    public LoginController(UserService userService, DefUserService defUserService, ISysBaseAPI sysBaseAPI, ISysLogService logService, RedisUtil redisUtil, ISysDepartService sysDepartService, ISysDictService sysDictService) {
...
//        String password = sysLoginModel.getPassword();
        ...
        User User = defUserService.getOne(queryWrapper);
    ...
    public Result<Object> logout(HttpServletRequest request) {
        //用户退出逻辑
        String token = request.getHeader(DefContents.X_ACCESS_TOKEN);
        ...
        if (User != null) {
            sysBaseAPI.addLog("用户名: " + User.getRealname() + ",退出成功！", CommonConstant.LOG_TYPE_1, null);
    ...
    public Result<JSONObject> selectDepart(@RequestBody User user) {
        Result<JSONObject> result = new Result<>();
        ...
        sysBaseAPI.getUserByName(username);
        JSONObject obj = new JSONObject();
    ...
    public Result<String> sms(@RequestBody JSONObject jsonObject) {
        ...
        //手机号模式 登录模式: "2"  注册模式: "1"
        val smsMode = jsonObject.get("smsmode").toString();
        log.info(mobile);
        ...
        try {
            ...
            //注册模板
            val User = userService.getUserByPhone(mobile);
            if (CommonConstant.SMS_TPL_TYPE_1.equals(smsMode)) {
                ...
                isSendSms = DySmsHelper.sendSms(mobile, obj, DySmsEnum.REGISTER_TEMPLATE_CODE);
            } else {
                //登录模式，校验用户有效性
                result = userService.checkUserIsEffective(User);
                if (!result.isSuccess()) {
                ...
                if (CommonConstant.SMS_TPL_TYPE_0.equals(smsMode)) {
                    //登录模板
                    isSendSms = DySmsHelper.sendSms(mobile, obj, DySmsEnum.LOGIN_TEMPLATE_CODE);
                } else if (CommonConstant.SMS_TPL_TYPE_2.equals(smsMode)) {
                    //忘记密码模板
                    isSendSms = DySmsHelper.sendSms(mobile, obj, DySmsEnum.FORGET_PASSWORD_TEMPLATE_CODE);
                }
            }
     ...
     * 用户信息
     */
    private void userInfo(User User, Result<JSONObject> result) {
        val password = User.getPassword();
        val username = User.getUsername();
        // 生成token
        val token = JwtUtil.sign(username, password);
        // 设置token缓存有效时间
    ...
    public Result<String> randomImage(@PathVariable String key) {
        ...
        try {
            ...
            val realKey = MD5Util.MD5Encode(lowerCaseCode + key, "utf-8");
    ...
    public Result<JSONObject> mLogin(@RequestBody SysLoginModel sysLoginModel) {
        Result<JSONObject> result;
        val username = sysLoginModel.getUsername();
        val password = sysLoginModel.getPassword();
        ...
        //2. 校验用户名或密码是否正确
        val userpassword = PasswordUtil.encrypt(username, password, User.getSalt());
        val syspassword = User.getPassword();
        if (!syspassword.equals(userpassword)) {
            result.error500("用户名或密码错误");
            return result;
        }
        ...
        // 生成token
        val token = JwtUtil.sign(username, syspassword);
        // 设置超时时间
        redisUtil.set(CommonConstant.PREFIX_USER_TOKEN + token, token);
        redisUtil.expire(CommonConstant.PREFIX_USER_TOKEN + token, JwtUtil.EXPIRE_TIME * 2 / 1000);
        //token 信息
    ...
    public Result<?> checkCaptcha(@RequestBody SysLoginModel sysLoginModel) {
        val captcha = sysLoginModel.getCaptcha();
        val checkKey = sysLoginModel.getCheckKey();
        if (captcha == null) {
            return Result.error("验证码无效");
        }
        val lowerCaseCaptcha = captcha.toLowerCase();
        val realKey = MD5Util.MD5Encode(lowerCaseCaptcha + checkKey, "utf-8");
