...
public class JedisViewMgr {
	...
	*/
	...
	public static <T extends JedisBaseView> long SaveView(String key, T... datas) throws JsonProcessingException
	{
		...
		return JedisMgr_wf.GetInstance().pushSortSetByStrMap(key, map);
	}
	...
	*/
	public static boolean SaveView(String key, String value, double score) throws JsonProcessingException
	{
		...
		return JedisMgr_wf.GetInstance().pushSortSet(key, value, score);
	}
	...
	*/
	...
	{
		return JedisMgr_wf.GetInstance().removeSortSet(key, values);
	}
	...
	*/
	...
	{
		Set<TypedTuple<String>> sets = JedisMgr_wf.GetInstance().getSortSetRevListWithScores(key, offset, count);
	...
	*/
	public static <T extends JedisBaseView> List<T> LoadView(String key, String min, String max, Class<T> clazz) throws InstantiationException, IllegalAccessException
	{
		...
		Set<String> sets = JedisMgr_wf.GetInstance().getSortSetListByLex(key, min, max);
		List<T> list = new ArrayList<T>();
		for(String str : sets)
		{
			T jbv = clazz.newInstance();
	...
	*/
	...
	{
		Set<String> sets = JedisMgr_wf.GetInstance().getSortSetListByLex(key, min, max, offset, count);
		List<T> list = new ArrayList<T>();
		for(String str : sets)
		{
			T jbv = clazz.newInstance();
	...
	*/
	...
	{
		Set<TypedTuple<String>> sets = JedisMgr_wf.GetInstance().getSortSetRevListWithScores(key, min, max, offset, count);
