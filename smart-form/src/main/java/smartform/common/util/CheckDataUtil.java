...
public class CheckDataUtil {
     ...
     * @param errorName
     */
    public static void checkNull(String text, String errorName) {
        if (isNull(text)) {
            throw new SmartFormError(errorName + "不能为空!");
        }
    }
     ...
     * @param isNull
     */
    public static void checkStringLenght(String text, int len, String errorName, boolean isNull) {
        if (!isNull) {
            checkNull(text, errorName);
        }
        if (text.length() > len) {
            throw new SmartFormError(errorName + "不能超过 " + len + " 个字!");
        }
    }
