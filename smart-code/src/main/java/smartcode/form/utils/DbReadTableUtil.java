...
public class DbReadTableUtil {
    ...
    public static List<String> a() throws SQLException {
        ...
        try {
            Class.forName(PathCodeUtils.b);
            b = DriverManager.getConnection(PathCodeUtils.c, PathCodeUtils.d, PathCodeUtils.e);
            c = b.createStatement(1005, 1007);
            if (PathCodeUtils.a.equals("mysql")) {
                var1 = MessageFormat.format("select distinct table_name from information_schema.columns where table_schema = {0}", cUtils.c(PathCodeUtils.f));
            }

            if (PathCodeUtils.a.equals("oracle")) {
                var1 = " select distinct colstable.table_name as  table_name from user_tab_cols colstable order by colstable.table_name";
            }

            if (PathCodeUtils.a.equals("postgresql")) {
                var1 = "select tablename from pg_tables where schemaname='public'";
            }

            if (PathCodeUtils.a.equals("sqlserver")) {
    ...
    public static List<ColumnVo> a(String var0) throws Exception {
        ...
        try {
            ...
            c = b.createStatement(1005, 1007);
            if (PathCodeUtils.a.equals("mysql")) {
                var2 = MessageFormat.format("select column_name,data_type,column_comment,numeric_precision,numeric_scale,character_maximum_length,is_nullable nullable from information_schema.columns where table_name = {0} and table_schema = {1} order by ORDINAL_POSITION", cUtils.c(var0), cUtils.c(PathCodeUtils.f));
            }

            if (PathCodeUtils.a.equals("oracle")) {
                var2 = MessageFormat.format(" select colstable.column_name column_name, colstable.data_type data_type, commentstable.comments column_comment, colstable.Data_Precision column_precision, colstable.Data_Scale column_scale,colstable.Char_Length,colstable.nullable from user_tab_cols colstable  inner join user_col_comments commentstable  on colstable.column_name = commentstable.column_name  where colstable.table_name = commentstable.table_name  and colstable.table_name = {0}", cUtils.c(var0.toUpperCase()));
            }

            if (PathCodeUtils.a.equals("postgresql")) {
                var2 = MessageFormat.format("select icm.column_name as field,icm.udt_name as type,fieldtxt.descript as comment, icm.numeric_precision_radix as column_precision ,icm.numeric_scale as column_scale ,icm.character_maximum_length as Char_Length,icm.is_nullable as attnotnull from information_schema.columns icm, (SELECT A.attnum,( SELECT description FROM pg_catalog.pg_description WHERE objoid = A.attrelid AND objsubid = A.attnum ) AS descript,A.attname FROM\tpg_catalog.pg_attribute A WHERE A.attrelid = ( SELECT oid FROM pg_class WHERE relname = {0} ) AND A.attnum > 0 AND NOT A.attisdropped  ORDER BY\tA.attnum ) fieldtxt where icm.table_name={1} and fieldtxt.attname = icm.column_name", cUtils.c(var0), cUtils.c(var0));
            }

            if (PathCodeUtils.a.equals("sqlserver")) {
                var2 = MessageFormat.format("select distinct cast(DataBaseUtils.name as varchar(50)) column_name,  cast(DataBaseConfig.name as varchar(50)) data_type,  cast(EModel.value as NVARCHAR(200)) comment,  cast(ColumnProperty(DataBaseUtils.object_id,DataBaseUtils.Name,'''Precision''') as int) num_precision,  cast(ColumnProperty(DataBaseUtils.object_id,DataBaseUtils.Name,'''Scale''') as int) num_scale,  DataBaseUtils.max_length,  (case when DataBaseUtils.is_nullable=1 then '''y''' else '''n''' end) nullable,column_id   from sys.columns DataBaseUtils left join sys.types DataBaseConfig on DataBaseUtils.user_type_id=DataBaseConfig.user_type_id left join (select top 1 * from sys.objects where type = '''U''' and name ={0}  order by name) cUtils on DataBaseUtils.object_id=cUtils.object_id left join sys.extended_properties EModel on EModel.major_id=cUtils.object_id and EModel.minor_id=DataBaseUtils.column_id and EModel.class=1 where cUtils.name={0} order by DataBaseUtils.column_id", cUtils.c(var0));
            }
            ...
            var6.setNullable(cUtils.a(var1.getString(7)));
            a(var6);
            ...
            if (PathCodeUtils.p != null) {
                var7 = PathCodeUtils.p.toLowerCase().split(",");
            }

            if (!PathCodeUtils.m.equals(var6.getFieldName()) && !DataBaseUtils.a(var6.getFieldDbName().toLowerCase(), var7)) {
                var3.add(var6);
            }
            ...
            while(var1.previous()) {
                ...
                if (!PathCodeUtils.m.equals(var8.getFieldName()) && !DataBaseUtils.a(var8.getFieldDbName().toLowerCase(), var7)) {
                    ...
                    var8.setNullable(cUtils.a(var1.getString(7)));
                    a(var8);
    ...
    public static List<ColumnVo> b(String var0) throws Exception {
        ...
        try {
            ...
            var6.setFieldDbName(var1.getString(1).toUpperCase());
            var6.setPrecision(cUtils.b(var1.getString(4)));
            var6.setScale(cUtils.b(var1.getString(5)));
            var6.setCharmaxLength(cUtils.b(var1.getString(6)));
            var6.setNullable(cUtils.a(var1.getString(7)));
            ...
            while(true) {
                ...
                var7.setPrecision(cUtils.b(var1.getString(4)));
                var7.setScale(cUtils.b(var1.getString(5)));
                var7.setCharmaxLength(cUtils.b(var1.getString(6)));
                var7.setNullable(cUtils.a(var1.getString(7)));
    ...
    public static boolean c(String var0) {
        ...
        try {
            a.debug("数据库驱动: " + PathCodeUtils.b);
            Class.forName(PathCodeUtils.b);
            b = DriverManager.getConnection(PathCodeUtils.c, PathCodeUtils.d, PathCodeUtils.e);
            c = b.createStatement(1005, 1007);
            if (PathCodeUtils.a.equals("mysql")) {
                var2 = "select column_name,data_type,column_comment,0,0 from information_schema.columns where table_name = '" + var0 + "' and table_schema = '" + PathCodeUtils.f + "'";
            }
